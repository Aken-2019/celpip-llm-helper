...
class TestHomePageView(DjangoE2ETestCase):
    def test_homepage_display(self):
        ...
        with sync_playwright() as playwright:
            ...
            # Check the title is displayed
            expect(
                self.page.get_by_role("heading", name="Welcome to Our Site")
            ).to_be_visible()
            expect(self.page.locator('h1:has-text("Welcome!")')).to_be_visible()
            expect(
                self.page.locator("#content p").filter(
                    has_text="This is the homepage content."
                )
            ).to_be_visible()

    @pytest.mark.skip(
        reason="Home page is automatically created in the database when visiting the"
    )
